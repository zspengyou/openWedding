...
(function (exports) {
  ...
  var GlobeScenes = function (nameArr, nameObj, delay) {
    this.nameArr = nameArr;//["0Asian","1NorthAmerica","2SouthAmerican","3Africa","4Europe","5Oceania","6Asian",]
  ...
  GlobeScenes.prototype.next = function () {
    ...
    if (this.index < this.N) {// number of regions
  ...
  GlobeScenes.prototype.scene = function (index) {//continent index number
    this.index = index;
    // if(index == 0){this.globe.clear();}

    var name = this.nameArr[this.index];//0Asian
    var urlObj = this.nameObj[name];
    var urlJson = urlObj.urlJson;//info.json
    var urls = urlObj.urls; // path to photos

    var h = (this.h = (180 + (360 * index) / this.N) % 360);

    var self = this;
    this.globeScence = new GlobeScene(urlJson, urls, self); //draw continent
  ...
  ///====================GlobeScene=========================///
  var GlobeScene = function (urlJson, urls, THIS) {
    //每个国家的播放、绘制过程

    this.urlJson = urlJson;//info.json
    this.urls = urls;//pathToPhotots
    this.EP = THIS.ep;
    ...
    this.ep = new EventProxy();
    this.index = 0;
    var N = (this.N = urls.length);// number of photos in a continent
    if (N) {
      this.loadJson();
      // this.next();
      this.ep.on("next", this.next.bind(this));
  ...
  //绘制地图上continent的路径
  GlobeScene.prototype.loadJson = function () {
    ...
    function success(data) {
      ...
      var lnglat = info.center;//"center":{"lat":-23,"lng":132},
      self.globe.viewAt(lnglat);

      self.region = info.areaName;//"澳洲",    
      window.text.hide();

      self.canvasDraws = new Draw.CanvasDraws(
        curvesData,
        bg,
        callback,
        options
  ...
  GlobeScene.prototype.next = function () {
    ...
    if (this.index < this.N) {// number of photos in a continent
      this.loadBroadCast();
      this.index += 1;
    } else {
      floatTag.clear();
