// Slider shows the current continent name, eg: Asia
!(function (root) {
  ...
  Slider.prototype.div = function () {
    var slider = (this.slider = $("<div></div>")
  ...
  Slider.prototype.divs = function (worldobj) {
    ...
    for (var name in worldobj) {
      //0Asian, 1NorthAmerica
      var urls = worldobj[name].urls;
      var len = urls.length; // number of pictures in a continent
    ...
    for (var k in lengthList) {
      ...
      var name = obj.name;
      name = name.substring(1, name.length); //move the number in the beginning of the name
      var len = obj.len;
...
//slider.select
  Slider.prototype.select = function (index, hue) {//hue
    this.cancelAll();

    hue = hue || 180;
    var grid = this.grids[index];
    var bgColor = Color({ hue: hue, saturation: 0.7, value: 0.6 }).toString();
    bgColor = "#fff";
    var span = grid.span;
    ...
    span.css("color", bgColor).css("opacity", 1);
  };
